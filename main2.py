...
if flagp == 0:
   ...
   E = np.linspace(0,4*h*wCO*cminv_to_Hz,5)
   ...
   for i in range(np.size(E)):
      ModeSe[i],TempInd[i],Deltak_k[i],kavg[i],kIVR[i] = RateChangeEachEi(E[i],t1part,t2part,zeta,w0,fmol,Nmol,Cloc,tIVR,tVC,Tout,A0,xB,tsplit)
   ...
   np.savetxt(fname,np.transpose([Ekcal,ModeSe,TempInd,Deltak_k,Pr,kavg,kIVR]))
   ...
   for i in range(np.size(E)):
      ModeSe[i],TempInd[i],Deltak_k[i],Tt = RateChangeEachEiCW(E[i],t,zeta,w0,fmol,Nmol,Cloc,tIVR,tVC,Tout,A0,xB,wD,trep)

   Ekcal = E*N_A/kcal_to_J
   # Save output to a file
   np.savetxt(fnameT,np.transpose([t,Tt-Tout]))
   np.savetxt(fname,np.transpose([Ekcal,ModeSe,TempInd,Deltak_k]))
